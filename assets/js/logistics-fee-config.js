...
function buildSeedData(){
  const base=[
    {name:'波兰仓-西欧专线-快递服务',customer:'深圳市星辰电子商务有限公司',channel:'西欧专线',serviceType:'快递',startDate:'2025-01-01',endDate:'2025-12-31',enabled:true,updater:'张三'},
    {name:'德国仓-全渠道-标准服务',customer:'杭州跨境通供应链管理有限公司',channel:'全渠道',serviceType:'标准',startDate:'2025-03-01',endDate:'2025-09-30',enabled:true,updater:'张三'},
    {name:'深圳仓-东南亚小包专线',customer:'广州优品国际贸易有限公司',channel:'东南亚专线',serviceType:'小包',startDate:'2025-02-15',endDate:'2025-08-15',enabled:false,updater:'李四'},
    {name:'波兰仓-北美专线-经济服务',customer:'上海锐达物流科技有限公司',channel:'北美专线',serviceType:'经济',startDate:'2025-04-01',endDate:'2026-03-31',enabled:true,updater:'李四'},
    {name:'德国仓-西欧专线-快递服务',customer:'深圳市星辰电子商务有限公司',channel:'西欧专线',serviceType:'快递',startDate:'2025-05-01',endDate:'2025-11-30',enabled:true,updater:'王五'},
    {name:'波兰仓-全渠道-标准服务',customer:'杭州跨境通供应链管理有限公司',channel:'全渠道',serviceType:'标准',startDate:'2025-06-01',endDate:'2025-12-31',enabled:true,updater:'张三'},
    {name:'深圳仓-西欧专线-经济服务',customer:'广州优品国际贸易有限公司',channel:'西欧专线',serviceType:'经济',startDate:'2025-01-15',endDate:'2025-07-15',enabled:false,updater:'王五'},
    {name:'德国仓-东南亚专线-小包服务',customer:'上海锐达物流科技有限公司',channel:'东南亚专线',serviceType:'小包',startDate:'2025-07-01',endDate:'2026-06-30',enabled:true,updater:'李四'},
    {name:'波兰仓-北美专线-标准服务',customer:'深圳市星辰电子商务有限公司',channel:'北美专线',serviceType:'标准',startDate:'2025-08-01',endDate:'2026-02-28',enabled:true,updater:'张三'},
    {name:'德国仓-全渠道-快递服务',customer:'杭州跨境通供应链管理有限公司',channel:'全渠道',serviceType:'快递',startDate:'2025-09-01',endDate:'2026-05-31',enabled:true,updater:'王五'},
  ];
  const rows=[];
...
const state={keyword:'',customer:'',channel:'',serviceType:'',status:'',currentPage:1,pageSize:10};
...
function getFilteredRows(){
  ...
  return rows.filter(item=>{
    ...
    if(state.serviceType&&item.serviceType!==state.serviceType)return false;
...
function queryList(){
  ...
  state.serviceType=serviceTypeSelect.value;
...
function resetList(){
  state.keyword='';state.customer='';state.channel='';state.serviceType='';state.status='';
  state.currentPage=1;state.pageSize=10;
  keywordInput.value='';customerSelect.value='';channelSelect.value='';
  serviceTypeSelect.value='';statusSelect.value='';pageSizeSelect.value='10';
  renderTable();
...
function toggleStatus(id){
  ...
  if(!window.confirm(next?'是否确认启用该报价单？':'是否确认停用该报价单？')){renderTable();return;}
  item.enabled=next;
  renderTable();
...
exportBtn.addEventListener('click',()=>{showToast('success','导出成功','报价单数据已导出。');});

tableBody.addEventListener('click',e=>{
  const t=e.target.closest('[data-action]');if(!t)return;
  const id=Number(t.dataset.id);
  if(t.dataset.action==='delete'){e.preventDefault();deleteRow(id);}
  if(t.dataset.action==='detail'){showToast('success','查看详情',`正在打开报价单详情...`);}
});
