...
const supplierTypeTag=document.getElementById('supplierTypeTag');
...
const SUPPLIER_OPTIONS=[
  {id:'S001',name:'DHL物流',type:'logistics'},
  {id:'S002',name:'UPS供应链',type:'logistics'},
  {id:'S003',name:'波兰本地仓',type:'warehouse'},
  {id:'S004',name:'欧洲操作中心',type:'operation'}
];
const SUPPLIER_TYPE_MAP={logistics:'物流供应商',warehouse:'仓储供应商',operation:'操作服务供应商'};

/* ── State ── */
const state={
  currentBizType:'trunk',
  currentCategory:'logistics',
  selectedSupplier:null,  // {id, name, type}
  selections:{},
...
if(isEdit||isView){
  ...
  schemeName.value='DHL-波兰仓干线物流报价';
  state.selectedSupplier={id:'S001',name:'DHL物流',type:'logistics'};
  supplierSelect.value='S001';
  supplierTypeTag.textContent='物流供应商';
...
function renderSummary(){
  ...
  const supplierName=state.selectedSupplier?state.selectedSupplier.name:'未选择';
  schemeSummary.innerHTML=`
    <div class="summary-items">
      <span>供应商: ${escapeHtml(supplierName)}</span>
      ${items.length?'<span style="color:var(--line)">|</span>':''}
...
const requiredFields={
  ...
  supplier:{el:supplierSelect,label:'供应商名称',check:()=>!state.selectedSupplier},
...
function validateForm(){
  ...
  for(const [key,cfg] of Object.entries(requiredFields)){
    if(cfg.check()){
      const el=cfg.el||supplierSelect;
...
feeSelectConfirmBtn.addEventListener('click',()=>{
  ...
  if(state.currentCategory==='storage'){
    // Get existing storage goodsTypes
...
if(previewBtn)previewBtn.addEventListener('click',()=>{QuotationPreview.open({schemeName:schemeName.value||'未命名方案',selectedCustomers:[state.selectedSupplier?state.selectedSupplier.name:''],warehouse:warehouse.value,startDate:startDate.value,endDate:endDate.value,selections:state.selections,priceOverrides:state.priceOverrides},allFeeItems);});
